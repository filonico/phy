...
We are also going to filter out orthogroups with more than 100 orthologs: this step is not required and most authors just take the OrthoFinder results as they are; though, it is true that the largest orthogroups may somethimes include just a bunch of random mis-annotated genes. In this case, we are filtering them out just to speed up the analysis.
```
